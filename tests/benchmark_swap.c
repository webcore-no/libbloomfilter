...
void worker_loop()
{
	...
	*k = rand();
	while (1) {
		int n = (*k) % 100;
		if (n < CONTAINS_P) {
			bloomfilterswap_test(globals.filter, key, key_len);
		} else if(n < CONTAINS_P+ADD_P) {
			bloomfilterswap_add(globals.filter, key, key_len);
		}
		else {
			bloomfilterswap_swap(globals.filter);
		}
		(*k)++;
		globals.op_counter++;
	}
...
int main(void)
{
	...
	globals.filter = bloomfilterswap_new(bloomfilter_shm_alloc);
	if (!globals.filter) {
	...
	for (int i = 0; i < WORKER_COUNT; i++) {
		if (create_worker(&workers[i])) {
			bloomfilterswap_destroy(&globals.filter,
	...
	bloomfilterswap_destroy(&globals.filter, bloomfilter_shm_free);
